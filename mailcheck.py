...
config = config.config('config_mail')

imap_host = config.get_item('mail','imap_host')
imap_user = config.get_item('mail','imap_user')
imap_pass = config.get_item('mail','imap_pass')

ALLOWED_SENDERS = config.get_item("mail","allowed_senders").split(',')
	...
	for text in splitMail:
		if "X-Mail-from:" in text:
			...
			if mail_address in ALLOWED_SENDERS:
				isSenderOK = True
		if text == config.get_item('mail','delivered_to'):
			isReceiverOK = True
